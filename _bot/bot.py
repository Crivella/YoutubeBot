...
@bot.hybrid_command(name="skip", aliases=["s"])
async def skip(ctx: commands.Context, n_skips: int = 1):
    try:
...
async def play(ctx: commands.Context, search: str):
    ...
    kwargs = yt_dl_kwargs.copy()
    kwargs["paths"]["home"] = f"./dl/{server_id}"
    kwargs["paths"]["home"] = f"./dl"
    ...
    with yt_dlp.YoutubeDL(kwargs) as ydl:
        try:
        ...
        if not os.path.exists(path):
            ...
            try:
                blocking = functools.partial(ydl.download, [query])
                await bot.loop.run_in_executor(None, blocking)
                # ydl.download([query])
            except yt_dlp.utils.DownloadError as err:
                await notify_about_failure(ctx, err)
                return
        else:
            await ctx.send(f'`{info["title"]}` already downloaded')
        try:
...
@bot.hybrid_command("loop", aliases=["l"])
...
@bot.hybrid_command("reset")
async def reset(ctx: commands.Context):
    ctx.send("resetting playlist queue and bot state...")
...
def after_track(error, connection, server_id):
    ...
    #         pass
    try:
        nxt = queues[server_id]["queue"][0][0]
...
async def on_voice_state_update(
    member: discord.User, before: discord.VoiceState, after: discord.VoiceState
):
    if member != bot.user:
        return
...
async def on_command_error(
    ctx: discord.ext.commands.Context, err: discord.ext.commands.CommandError
):
